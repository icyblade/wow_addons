			...
			if player ~= nil and (not Spy.InInstance or zone == GetZoneText()) then
				if not Spy.PlayerCommList[player] then
					...
					if strlen(race) > 0 then
						...
						if (Spy.EnemyFactionName == "Alliance" and race ~= "Draenei" and race ~= "Dwarf" and race ~= "Gnome" and race ~= "Human" and race ~= "Night Elf" and race ~= "Worgen")
						or (Spy.EnemyFactionName == "Horde" and race ~= "Blood Elf" and race ~= "Goblin" and race ~= "Orc" and race ~= "Tauren" and race ~= "Troll" and race ~= "Undead") then
							return
						end
					else
						race = nil
...
function Spy:ShowMapNote(player)
	...
	if playerData then
		...
		if continentIndex ~= nil and zoneIndex ~= nil and type(playerData.mapX) == "number" and type(playerData.mapY) == "number" and (Spy.db.profile.MapDisplayLimit == "None" or (Spy.db.profile.MapDisplayLimit == "SameZone" and zoneIndex == currentZoneIndex) or (Spy.db.profile.MapDisplayLimit == "SameContinent" and continentIndex == currentContinentIndex)) then
			...
			Astrolabe:PlaceIconOnWorldMap(WorldMapDetailFrame, note.worldIcon, Astrolabe:GetMapID(continentIndex, zoneIndex), 1, mapX, mapY)
			Astrolabe:PlaceIconOnMinimap(note.miniIcon, Astrolabe:GetMapID(note.continentIndex, note.zoneIndex), 1, note.mapX, note.mapY)
