...
local PreviousFrameUndo = nil


local function SetColumn(column, previous)
	ColumnFrame = column or ColumnFrame
	PreviousFrame = previous or PreviousFrame
end

local function SetActivePanel(frame)
	PanelFrame = frame
end

local function SetOffset(label)
	if not label then return end
	PreviousFrameUndo = PreviousFrame
	PreviousFrame = PanelFrame[label]
end

local function UndoOffset()
	PreviousFrame = PreviousFrameUndo
end


--local function QuickSetPoints(frame, columnFrame, neighborFrame, xOffset, yOffset)
	local function QuickSetPoints(frame, xOffset, yOffset)
		...
		if PreviousFrame then
			if PreviousFrame.Margins then TopOffset = PreviousFrame.Margins.Bottom + TopOffset + (yOffset or 0) end
			frame:SetPoint("TOP", PreviousFrame, "BOTTOM", -(PreviousFrame:GetWidth()/2), -TopOffset)
	...
	local function CreateQuickSlider(var, label, ... ) --, neighborFrame, xOffset, yOffset)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		local frame = PanelHelpers:CreateSliderFrame(globalName, ColumnFrame, L(label), .5, 0, 1, .1)
		frame:SetWidth(250)
		...
		frame:SetScript("OnMouseUp", function()
			--OnPanelItemChange()
			frame:GetParent().Callback()
			--ColumnFrame.Callback()
			--if ColumnFrame.OnFeedback then ColumnFrame:OnFeedback() end
		end)

		PanelFrame[var] = frame
		return frame
	end

	local function CreateQuickCheckbutton(var, label, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		local frame = PanelHelpers:CreateCheckButton(globalName, ColumnFrame, L(label))
		--frame.Label:SetFont("FONTS/ARIALN.TTF", 14)
	...
	local function CreateQuickEditbox(var, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		local frame = CreateFrame("ScrollFrame", globalName, ColumnFrame, "UIPanelScrollFrameTemplate")
	...
	local function CreateQuickColorbox(var, label, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		local frame = PanelHelpers:CreateColorBox(globalName, ColumnFrame, L(label), 0, .5, 1, 1)
		-- Margins	-- Bottom/Left are supposed to be negative
	...
	local function CreateQuickDropdown(var, label, dropdownTable, initialValue, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		local frame = PanelHelpers:CreateDropdownFrame(globalName, ColumnFrame, dropdownTable, initialValue, L(label))
		-- Margins	-- Bottom/Left are supposed to be negative
	...
	local function CreateQuickHeadingLabel(var, label, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame

		print(globalName)
		local frame = CreateFrame("Frame", globalName, ColumnFrame)
		-- Heading Appearance
		frame:SetHeight(26)
		frame:SetWidth(500)
...
--[[
	local function CreateDrawer(var, label, ...)
		--local globalName = panel.objectName..name	-- panel, var  -- replaces name,		-- panel[var] = frame
--PanelFrame[var] = frame
	...
	local function CreateQuickItemLabel(var, label, ...)
		local globalName = PanelFrame.objectName..var	-- panel, var  -- replaces name,		-- panel[var] = frame
...
TidyPlatesHubRapidPanel.SetColumn = SetColumn
TidyPlatesHubRapidPanel.SetActivePanel = SetActivePanel
TidyPlatesHubRapidPanel.SetOffset = SetOffset
TidyPlatesHubRapidPanel.UndoOffset = UndoOffset
...
local function CreateInterfacePanel( objectName, panelTitle, parentTitle)
	...
	------------------------------
	SetColumn(panel, nil)
	CreateQuickHeadingLabel("MainLabel", panelTitle, 16, 8)
	...
	panel.WarningFrame:Hide()
        -- Description
        SetColumn(panel.WarningFrame, nil)
        CreateQuickHeadingLabel("Warnings", "", 8, -4)
        -- Button
