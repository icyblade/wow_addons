...
-- local ITEM_TYPE_QUEST = select(10, GetAuctionItemClasses()) -- ICY: no such API
local ITEM_TYPE_QUEST = {'Quest', '任务', '任務'}
...
local function HandleItems(parserEvent)
 ...
 -- if ((itemType == ITEM_TYPE_QUEST) and currentProfile.alwaysShowQuestItems) then showEvent = true end -- ICY: no such API
 for _, icy_type in pairs(ITEM_TYPE_QUEST) do
  if itemType == icy_type and currentProfile.alwaysShowQuestItems then
   showEvent = true
  end
 end
