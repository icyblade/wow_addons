...
Special thanks: 
		...
		lsjyzjl (Traditional Chinese Translations)
		Phalk and Omukeka (Brazilian Portuguese Translations)
		Andyca (Traditional Chinese Translations)
...
Change Log:
	...
	v4.12.4
		...
		- Updated French localization - Thanks Blubibulga
	v4.13
		- Added support for Mists of Panderia (5.0.1)
		- Added "INSTAKILL" event for fail alerts
		- Added "Fatigue" to "special alerts"
		- Added Cataclysm spells for Dragon Soul (Heroic Hagara)
		- Added Panderia spells for Scholomance
		- Added Panderia spells for Scarlet Cathedral
		- Added Panderia spells for Scarlet Halls
		- Added Panderia spells for Stormstout Brewery
		- Added Panderia spells for Temple of the Jade Serpent
...
GTFO = {
	...
	Version = "4.13.0"; -- Version number (text format)
	VersionNumber = 41300; -- Numeric version number for checking out-of-date clients
...
function GTFO_OnEvent(self, event, ...)
	...
	if (event == "COMBAT_LOG_EVENT_UNFILTERED") then
				...
				if (sourceGUID == UnitGUID("player")) then
					...
					if (SpellType=="SPELL_DAMAGE") then
						...
						if (GTFO.FFSpellID[SpellID]) then
							...
							local damage = misc4 ~= "ABSORB" and misc4 or 0
		...
		if (SpellType == "ENVIRONMENTAL_DAMAGE") then
			...
			local damage = misc2 ~= "ABSORB" and misc2 or 0
			local damagePercent = (damage * 100) / UnitHealthMax("player")
		...
		elseif (SpellType=="SPELL_PERIODIC_DAMAGE" or SpellType=="SPELL_DAMAGE" or SpellType=="SPELL_MISSED" or SpellType=="SPELL_PERIODIC_MISSED" or SpellType=="SPELL_ENERGIZE" or SpellType=="SPELL_INSTAKILL" or ((SpellType=="SPELL_AURA_APPLIED" or SpellType=="SPELL_AURA_APPLIED_DOSE" or SpellType=="SPELL_AURA_REFRESH") and misc4=="DEBUFF")) then
			...
			local damage = tonumber(misc4 ~= "ABSORB" and misc4 or 0)
	...
	if (event == "PARTY_MEMBERS_CHANGED") then
		-- Remove this after MoP is released
		GTFO_DebugPrint("Party roster was updated");
		GTFO_ScanGroupGUID();
	...
	if (event == "RAID_ROSTER_UPDATE") then
		-- Remove this after MoP is released
		GTFO_DebugPrint("Raid roster was updated");
		GTFO_ScanGroupGUID();
...
function GTFO_RecordStats(alertID, SpellID, SpellName, damage, sourceName)
	if (alertID > 0 and (GTFO.Recount or GTFO.Skada)) then
