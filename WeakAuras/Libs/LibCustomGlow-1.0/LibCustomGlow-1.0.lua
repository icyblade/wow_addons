...
local Retail = LibStub("LibRetail", true)

local pairs, ipairs, next, select, CreateFrame = pairs, ipairs, next, select, CreateFrame
...
local GlowMaskPool = Retail.CreateFromMixins(Retail.ObjectPoolMixin)
lib.GlowMaskPool = GlowMaskPool
local function MaskPoolFactory(maskPool)
    return maskPool.parent:CreateTexture("OVERLAY")
end
...
Retail.ObjectPoolMixin.OnLoad(GlowMaskPool,MaskPoolFactory,MaskPoolResetter)
...
local GlowTexPool = Retail.CreateTexturePool(GlowParent ,"ARTWORK",7,nil,TexPoolResetter)
...
local GlowFramePool = Retail.CreateFramePool("Frame",GlowParent,nil,FramePoolResetter)
...
table_insert(lib.glowList, "Pixel Glow")
...
table_insert(lib.glowList, "Autocast Shine")
...
local ButtonGlowPool = Retail.CreateFramePool("Frame",GlowParent,nil,ButtonGlowResetter)
...
table_insert(lib.glowList, "Action Button Glow")
