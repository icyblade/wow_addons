...
--		/dump DETAILS_PLUGIN_ENCOUNTER_DETAILS.db.show_icon
	...
	function EncounterDetails:CreateGraphPanel()
			...
			local phase_on_enter = function (self)
				...
				if (combat) then
					...
					for player_name, damage in pairs (combat.PhaseData.damage [self.phase]) do
						tinsert (damages, {player_name, damage})
					end
					table.sort (damages, _detalhes.Sort2)

					for i, player in ipairs (damages) do
					...
					local damage_players = #damages
					self.damage_actors = damages

					--heal
					local heals = {}
					for player_name, heal in pairs (combat.PhaseData.heal [self.phase]) do
						tinsert (heals, {player_name, heal})
					end
					table.sort (heals, _detalhes.Sort2)

					for i, player in ipairs (heals) do
					...
					local heal_players = #heals
					self.heal_actors = heals
		...
		--> chart frame implementations
			...
			for i = 1, 8, 1 do
				...
				line:SetTexture (unpack (CONST_DAMAGE_LINES_COLOR))
	...
	BossFrame.buttonSwitchNormal.MouseOnLeaveHook = function()
		_detalhes.popup:ShowMe (false)
--		t:SetBlendMode ("BLEND")
	...
	BossFrame.buttonSwitchGraphic.MouseOnLeaveHook = function()
		_detalhes.popup:ShowMe (false)
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	--> Selecionar o segmento
		...
		local buildSegmentosMenu = function (self)
			...
			for index, combate in ipairs (historico) do
				if (combate.is_boss and combate.is_boss.index and combate.is_boss.name) then
	...
	-- ~adds �dds
		BOX_WIDTH = 270
		BOX_HEIGHT = CONST_BOX_HEIGHT --173
